...
class UpliftRandomForestDML:
    """Random forest of DML uplift trees.

    The estimator fits multiple uplift trees on bootstrap samples and random
    feature subsets, then averages their predicted uplift values.
    """
    ...
    def __init__(
        ...
        if not 0 < self.max_features <= 1:
            raise ValueError("max_features must be in the interval (0, 1].")
        self.max_samples = max_samples
        if not 0 < self.max_samples <= 1:
            raise ValueError("max_samples must be in the interval (0, 1].")
    ...
    def _preprocess_dml(self, X: pd.DataFrame, y: np.ndarray, w: np.ndarray):
        ...
        mu_model = RandomForestRegressor(
            n_estimators=50, max_depth=6, random_state=self.random_state
        )
    ...
    def fit(
        ...
        for i in range(self.n_estimators):
            ...
            cols_idx = rng.sample(init_cols, cols_sample_cnt)
            rows_idx = rng.choices(init_rows, k=rows_sample_cnt)
        ...
        if self.n_jobs is not None and self.n_jobs != 1:
            if self.verbose > 0:
                print(
                    f"[UpliftRandomForest] Fitting {self.n_estimators} trees in parallel (n_jobs={self.n_jobs})..."
                )
            ...
            if self.verbose > 0:
                print(
                    f"[UpliftRandomForest] Fitting {self.n_estimators} trees sequentially..."
                )
    ...
    def predict(self, X: pd.DataFrame) -> np.ndarray:
        """Predict uplift values by averaging predictions across fitted trees."""
        if not self._fitted:
        ...
        return predictions / len(self.forest)
