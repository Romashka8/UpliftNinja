...
def upliftComparingHist(
    model_name_1, model_predictions_1, model_name_2, model_predictions_2
):
...
def scoreUpliftAtK(model_w_predictions, target_test, treatment_test, k=0.2):
    results = {}
    for model_name, predictions in model_w_predictions:
        up_k_best = uplift_at_k(
            y_true=target_test,
            uplift=predictions,
...
def scorePipelines(
    ...
    for pipeline in pipelines_unfited:
        ...
        if "treeCausal" in steps:
            pipelines_unfited[pipeline] = pipelines_unfited[pipeline].fit(
                data_train, treatment_train, target_train
            )
        else:
            pipelines_unfited[pipeline] = pipelines_unfited[pipeline].fit(
...
def uplift_score(
    prediction: np.array, treatment: np.array, target: np.array, rate: float = 0.3
) -> float:
    """
    Calculate Uplift Score:
        -Made special for X5 Retail Hero
    """
    order = np.argsort(-prediction)
    treatment_n = int((treatment == 1).sum() * rate)
    treatment_p = target.iloc[order][treatment.iloc[order] == 1][:treatment_n].mean()
    control_n = int((treatment == 0).sum() * rate)
    control_p = target.iloc[order][treatment.iloc[order] == 0][:control_n].mean()
